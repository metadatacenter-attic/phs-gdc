 ...
 * @param places    A list of Place DCIDs to query for (Here DCID stands for Data Commons ID, the unique identifier
 *                  assigned to all entities in Data Commons.)
 * @param statVars  A list of StatisticalVariable DCIDs.
 * @constructor
 */
export function getPlaceStatistics(placeType, placeValues, statVars) {
  let url = DC_API_BASE_URL + "stat/all";
  const requestOptions = {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({places: placeValues, statVars: statVars})
  };
